...
function _paintTileLabels(c, easedZoomLevel, firstVisibleLocalX, lastVisibleLocalX, firstVisibleLocalY, lastVisibleLocalY) {
...
Painter.prototype = {
  ...
  _paintTileContents: function (c, floorTimeSignal, roundZoomPower, groupCount, groupSize, firstVisibleGroupX, lastVisibleGroupX, firstVisibleGroupY, lastVisibleGroupY) {
    for (var gx = firstVisibleGroupX; gx <= lastVisibleGroupX; gx += groupCount) {
      ...
      for (var gy = firstVisibleGroupY; gy <= lastVisibleGroupY; gy += groupCount) {
        ...
        var groupId = iid.fromLocal(gx, gy, floorTimeSignal, roundZoomPower);
  ...
  _paint: function (state) {
    ...
    if (state.easedZoomPower < 3) {
      c.globalAlpha = 1 - (state.easedZoomPower - 2);
      _paintTileLabels(c, state.easedZoomLevel, state.firstVisibleLocalX, state.lastVisibleLocalX, state.firstVisibleLocalY, state.lastVisibleLocalY);
      c.globalAlpha = 1;
    }
    ...
    c.translate(-state.scrollLeft, -state.scrollTop);
    c.scale(1 / state.easedZoomLevel, 1 / state.easedZoomLevel);
    this._paintTileContents(c, state.floorTimeSignal, state.roundZoomPower, state.groupCount, state.groupSize, state.firstVisibleGroupX, state.lastVisibleGroupX, state.firstVisibleGroupY, state.lastVisibleGroupY);
    c.restore();
    c.save();
    c.translate(0.5 / window.devicePixelRatio, 0.5 / window.devicePixelRatio);
    this._pendingPaint = false;
