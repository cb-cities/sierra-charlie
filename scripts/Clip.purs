...
leftProduct r l d = (r.xleft - l.x1) * d.y

-- TODO
topProduct :: Rect -> Line -> Point -> Number
topProduct r l d = (r.ytop - l.y1) * d.x

-- TODO
rightProduct :: Rect -> Line -> Point -> Number
rightProduct r l d = (r.xright - l.x1) * d.y

-- TODO
bottomProduct :: Rect -> Line -> Point -> Number
bottomProduct r l d = (r.ybottom - l.y1) * d.x
...
clip r l | l.x1 < r.xleft  = clipP1Left r l
         | l.x1 > r.xright = rotateLine180c <$> clipP1Left (rotateRect180c r) (rotateLine180c l)
         | otherwise       = clipP1Centre r l

-- 1. "leftcolumn"
-- P1 is beyond the left boundary
clipP1Left :: Rect -> Line -> Maybe Line
clipP1Left r l | l.x2 < r.xleft   = Nothing
               | l.y1 > r.ytop    = clipP1LeftTop r l
               | l.y1 < r.ybottom = reflectLineXAxis <$> clipP1LeftTop (reflectRectXAxis r) (reflectLineXAxis l)
...
clipP1LeftTop r l | l.y2 > r.ytop = Nothing
...
leftBottomRegion r l d pleft | l.y2 < r.ybottom = lbrP2Bottom r l d pleft (bottomProduct r l d)
...
lbrP2Inside r l d pleft | l.x2 > r.xright = let pright = rightProduct r l d in
                                            { x : r.xright
                                            , y : l.y1 + pright / d.x
                                            }
                        | otherwise       = { x : l.x2
...
lbrP2Bottom' r l d pbottom | l.x2 > r.xright = lbrP2BottomRight r l d pbottom (rightProduct r l d)
                           | otherwise       = { x : l.x1 + pbottom / d.y
                                               , y : r.ybottom
                                               }

-- P2 is to the right of the right boundary
lbrP2BottomRight :: Rect -> Line -> Point -> Number -> Number -> Point
lbrP2BottomRight r l d pbottom pright | pbottom > pright = { x : l.x1 + pbottom / d.y
                                                           , y : r.ybottom
                                                           }
...
clipP1LeftMiddle r l | l.y2 < r.ybottom = clipP1LeftMiddleP2Bottom r l
                     | l.y2 > r.ytop    = reflectLineXAxis <$> clipP1LeftMiddleP2Bottom (reflectRectXAxis r) (reflectLineXAxis l)
...
clipP1LeftMiddleP2Middle r l d | l.x2 > r.xright = let pright = rightProduct r l d in
                                                   { x : r.xright
                                                   , y : l.y1 + pright / d.x
                                                   }
                               | otherwise       = { x : l.x2
...
clipP1LeftMiddleP2Bottom'' r l d pbottom | l.x2 > r.xright = clipP1LeftMiddleP2RightBottom r l d pbottom (rightProduct r l d)
                                         | otherwise       = { x : l.x1 + pbottom / d.y
                                                             , y : r.ybottom
                                                             }
...
clipP1LeftMiddleP2RightBottom r l d pbottom pright | pbottom > pright = { x : l.x1 + pbottom / d.y
                                                                        , y : r.ybottom
                                                                        }
                                                   | otherwise        = { x : r.xright
                                                                        , y : l.y1 + pright / d.x
...
clipP1Centre r l | l.y1 < r.ybottom = clipP1CentreBottom r l
                 | l.y1 > r.ytop    = clipP1CentreTop r l
...
clipP1CentreBottom r l | l.y2 < r.ybottom = Nothing
...
clipP1CentreTop r l | l.y2 > r.ytop = Nothing
...
clipP1CentreMiddle r l | l.x2 < r.xleft   = clipP1CentreMiddleP2Left r l
                       | l.x2 > r.xright  = rotatePoint180c $ clipP1CentreMiddleP2Left (rotateRect180c r) (rotateLine180c l)
                       | l.y2 > r.ytop    = let d    = delta l
                                                ptop = topProduct r l d in
                                            { x : l.x1 + ptop / d.y
                                            , y : r.ytop
                                            }
                       | l.y2 < r.ybottom = let d       = delta l
...
clipP1CentreMiddleP2Left r l | l.y2 > r.ytop    = clipP1CentreMiddleP2LeftTop r l
                             | l.y2 < r.ybottom = rotatePoint270c $ clipP1CentreMiddleP2LeftTop (rotateRect90c r) (rotateLine90c l)
                             | otherwise        = let d     = delta l
...
clipP1CentreMiddleP2LeftTop' r l d pleft ptop | ptop > pleft = { x : l.x1 + ptop / d.y
                                                               , y : r.ytop
                                                               }
                                              | otherwise    = { x : r.xleft
                                                               , y : l.y1 + pleft / d.x
